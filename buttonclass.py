...
class Button:
    ...
    def __init__(self, win, center, width, height, colorB, label):
    ...
    def isClicked(self, p):
        ...
        return (self.active) and (p.getX() >= self.xmin and p.getX() <= self.xmax and p.getY() >= self.ymin and p.getY() <= self.ymax)
